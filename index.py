...
def copy_data_to_template(df, sheet_name, selected_name, template_file):
    ...
    # Escribir los datos filtrados en la plantilla de la hoja correspondiente
    sheet = template[sheet_name]
    for row in dataframe_to_rows(df_filtered, index=False, header=True):
        sheet.append(row)

    # Guardar el archivo Excel modificado en un buffer en memoria
    with BytesIO() as output:
        template.save(output)
        output.seek(0)
        return output.getvalue()

# Función para convertir los datos de un archivo Excel a CSV
def convert_to_csv(excel_file):
    # Cargar el archivo Excel modificado
    df = pd.read_excel(excel_file, sheet_name=None)  # Cargar todas las hojas
    output = BytesIO()
...
if uploaded_file is not None:
    ...
    if st.button('Exportar Hoja O'):
        ...
        file_o_csv = convert_to_csv(file_o)
        st.download_button("Descargar Hoja O como CSV", data=file_o_csv, file_name="plantilla_O.csv")
    ...
    if st.button('Exportar Hoja DP'):
        ...
        file_dp_csv = convert_to_csv(file_dp)
        st.download_button("Descargar Hoja DP como CSV", data=file_dp_csv, file_name="plantilla_DP.csv")
    ...
    if st.button('Exportar Hoja STD'):
        ...
        file_std_csv = convert_to_csv(file_std)
        st.download_button("Descargar Hoja STD como CSV", data=file_std_csv, file_name="plantilla_STD.csv")
